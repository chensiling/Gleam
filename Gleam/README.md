...
检查:
  ...
  find <addr> <size> ascii|utf16 <txt> 字符串搜索
...
事件暂停:
  breakon [sw] [on|off]                暂停开关：entry/dll/thread/exception/debugstring

异常:
  ignoreexc <hexcode>                  将指定异常码交还目标处理
  exception pass|handle                当前异常停止的处置（pass=交还目标 SEH，
                                       handle=DBG_CONTINUE 吞掉）
  excfilter [add|del|list]             异常过滤器管理
    add <code> [first|second|never] [pass|swallow]
    del <code>
    (裸 excfilter 为列表)
```
...
├── GleamDebugger.h/.cpp               调试器类：事件回调、命令循环（暂停态 + 运行态）、
│                                      逻辑断点绑定/解绑、running-command 队列管理
...
├── GleamCommands.Env.cpp              进程/线程环境（peb/teb/tls/threadtime）
├── GleamCommands.Hide.cpp             反反调试
└── GleamCommands.Scan.cpp             代码扫描（xref/findasm）

GleeBug/                               上游调试引擎（x64dbg 官方）
├── Debugger.h/.cpp                    引擎核心：进程/线程/断点管理
├── Debugger.Loop.cpp                  WaitForDebugEvent 循环 + cbOnTimeout 超时回调
└── Debugger.*.cpp                     事件分发、内存/寄存器操作、PE 解析等
```
