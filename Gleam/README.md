...
33 个场景 89 项断言，覆盖断点、执行控制、内存/寄存器读写、反汇编、符号枚举、异常过滤、脱离、跟踪、补丁、Release/FPO 栈展开及 pause 注入压力（25 连发）。`TestTarget` 工程是被调试目标，固定基址（无 ASLR）保证地址稳定可断言。脚本在任一断言失败时以非零退出码结束。
