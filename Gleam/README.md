...
执行控制:
  ...
  ret [max]               执行到当前函数返回（stepout 核心循环：遇 ret 执行后停在调用者；
                          call 用一次性断点跳过；后向跳转的循环原生速度快进；不依赖栈/展开数据）
  pause                   中断运行中的目标
  detach                  脱离调试（先 pause）
...
58 个场景 131 项断言，覆盖断点、执行控制、内存/寄存器读写、反汇编、符号枚举、异常过滤、脱离、跟踪、补丁全矩阵、argv 引用、跨块扫描、Release/FPO 栈展开及压力（100 连发 pause 注入、100 会话）。`TestTarget` 等工程是被调试目标，固定基址保证地址稳定可断言。脚本在任一断言失败时以非零退出码结束。
