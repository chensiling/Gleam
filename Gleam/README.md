...
52 个场景 126 项断言，覆盖断点、执行控制、内存/寄存器读写、反汇编、符号枚举、异常过滤、脱离、跟踪、补丁全矩阵、argv 引用、跨块扫描、Release/FPO 栈展开及压力（100 连发 pause 注入、100 会话）。`TestTarget` 等工程是被调试目标，固定基址保证地址稳定可断言。脚本在任一断言失败时以非零退出码结束。
